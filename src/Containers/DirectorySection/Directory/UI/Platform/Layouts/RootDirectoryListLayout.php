...
class RootDirectoryListLayout extends Table
{
    /**
     * Data source.
     *
    ...
    protected function columns(): array
    {
        return [
            ...
            TD::make('id', __('ID'))
                ...
                ->width(50)
                ->sort()
                ->render(fn (Directory $item) => Link::make((string) $item->id)->route(Directory::ROUTE_ROOT_EDIT, $item->id)),

            // custom columns
            TD::make('name', __('Name'))
                ->render(fn (Directory $item) => Link::make($item->name)->route(Directory::ROUTE_CHILD_LIST, ['root' => $item->id])),
            ...
            TD::make('actions', __('Actions'))
                ...
                ->render(function (Directory $item) {
                    return DropDown::make()
                        ...
                        ->list([
                            Link::make(__('Edit'))
                                ->route(Directory::ROUTE_ROOT_EDIT, $item->id)
                                ->icon('pencil'),
